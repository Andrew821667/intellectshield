...
class LOFDetector(BaseAnomalyDetector):
    """
    Детектор аномалий на основе алгоритма Local Outlier Factor.
    """
    
    def preprocess_data(self, data, train=False):
        """
        Предобработка данных для LOF.
        """
    ...
    def train(self, data, contamination=0.05, n_neighbors=20, metric='euclidean'):
        """
        Обучение модели LOF.
        """
        print("Начало обучения модели Local Outlier Factor...")
        ...
        # Инициализация и обучение модели LOF
        self.model = LocalOutlierFactor(
            n_neighbors=n_neighbors,
            contamination=contamination,
            metric=metric,
            novelty=True,  # Настраиваем для использования в режиме новизны
            n_jobs=-1
        )
        
        # Обучение модели (в LOF это фактически fit_predict, но мы игнорируем результаты)
    ...
    def predict(self, data):
        """
        Обнаружение аномалий с помощью LOF.
        """
        ...
        # Получение аномальных оценок (в LOF с novelty=True это отрицательные оценки)
        ...
        result_df['anomaly_score'] = anomaly_scores  # Уже инвертированы выше
