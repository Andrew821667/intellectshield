...
class EnhancedSQLInjectionDetectorV2(BaseAnomalyDetector):
    """
    Улучшенная версия 2 детектора SQL-инъекций с оптимизированным белым списком
    и расширенными паттернами для обнаружения типичных инъекций.
    
    Основные улучшения:
    1. Оптимизированный белый список для снижения пропусков инъекций
    2. Расширенные паттерны для обнаружения стандартных атак
    3. Улучшенный алгоритм принятия решений
    4. Прямая проверка наиболее распространенных векторов атак
    """
    
    def __init__(self, 
                 text_column: str = 'query', 
                 threshold: float = 0.55,  # Чуть снижен порог для лучшего баланса
        ...
        self.weights = {
            'regex': 0.60,        # Еще больше увеличен вес регулярных выражений
            'entropy': 0.15,
            'heuristics': 0.20,   # Немного снижен вес
            'context': 0.05
        }
        ...
        # Высокоприоритетные паттерны - однозначные признаки SQL-инъекции
        # Эти паттерны имеют преимущество перед белым списком
        self.critical_patterns = [
            # OR 1=1 - классические boolean-based инъекции
            r"(?i)['\"]?\s+OR\s+['\"]?[0-9]+['\"]?\s*=\s*['\"]?[0-9]+['\"]?",
            r"(?i)['\"]?\s+OR\s+['\"]?[\w]+['\"]?\s*=\s*['\"]?[\w]+['\"]?",
            
            # Комментарии с одинарной кавычкой
            r"(?i)'\s*--",
            r"(?i)'\s*#",
            
            # UNION SELECT - явный признак UNION-based инъекции
            r"(?i)'\s*UNION\s+SELECT",
            r"(?i)['\"]?\s+UNION\s+SELECT",
            
            # Точка с запятой и опасная команда
            r"(?i)'\s*;\s*DROP",
            r"(?i)'\s*;\s*DELETE",
            r"(?i)'\s*;\s*UPDATE",
            r"(?i)'\s*;\s*INSERT",
            ...
            # Специфические функции для слепых инъекций
        ...
        # Компилируем критические паттерны
        ...
        # Белый список стандартных SQL-запросов (оптимизированный)
        # Более строгие паттерны для исключения возможных инъекций
        self.whitelist_patterns = [
            # Стандартный SELECT с числовыми ID
            r"(?i)^SELECT\s+[\w\s,\*\(\)\.]+\s+FROM\s+[\w\.]+\s+WHERE\s+[\w\.]+\s*=\s*\d+$",
            
            # INSERT с четко определенной структурой
            r"(?i)^INSERT\s+INTO\s+[\w\.]+\s*\([\w\s,]+\)\s*VALUES\s*\([^']*\)$",
            
            # INSERT с безопасными строковыми значениями (не содержащими OR, UNION, и т.д.)
            r"(?i)^INSERT\s+INTO\s+[\w\.]+\s*\([\w\s,]+\)\s*VALUES\s*\([^']*'(?!.*(?:OR|UNION|SELECT|DROP|ALTER|DELETE))[\w\s@\.,]+'\s*(?:,\s*'(?!.*(?:OR|UNION|SELECT|DROP|ALTER|DELETE))[\w\s@\.,]+'\s*)*\)$",
            
            # UPDATE с числовым ID
            r"(?i)^UPDATE\s+[\w\.]+\s+SET\s+[\w]+\s*=\s*\d+\s+WHERE\s+[\w\.]+\s*=\s*\d+$",
            
            # UPDATE с безопасными строковыми значениями
            r"(?i)^UPDATE\s+[\w\.]+\s+SET\s+[\w]+\s*=\s*'(?!.*(?:OR|UNION|SELECT|DROP|ALTER|DELETE))[\w\s@\.,]+'\s+WHERE\s+[\w\.]+\s*=\s*\d+$",
            
            # DELETE с числовым ID
            r"(?i)^DELETE\s+FROM\s+[\w\.]+\s+WHERE\s+[\w\.]+\s*=\s*\d+$",
            
            # Простые COUNT запросы
            r"(?i)^SELECT\s+COUNT\(\*\)\s+FROM\s+[\w\.]+(?:\s+WHERE\s+[\w\.]+\s*(?:=|>|<|>=|<=)\s*\d+)?$",
            
            # Простые JOIN запросы с числовыми условиями
            r"(?i)^SELECT\s+[\w\s,\*\(\)\.]+\s+FROM\s+[\w\.]+\s+JOIN\s+[\w\.]+\s+ON\s+[\w\.]+\s*=\s*[\w\.]+(?:\s+WHERE\s+[\w\.]+\s*(?:=|>|<|>=|<=)\s*\d+)?$"
        ]
        ...
        self.logger.info(f"EnhancedSQLInjectionDetectorV2 initialized with threshold={threshold}")
    ...
    def train(self, data: pd.DataFrame, **kwargs) -> 'EnhancedSQLInjectionDetectorV2':
        """
    ...
    def fit(self, data: pd.DataFrame, **kwargs) -> 'EnhancedSQLInjectionDetectorV2':
        """
        Обучение детектора на нормальных данных.
        """
    ...
    def _check_critical_patterns(self, text: str) -> Tuple[bool, List[str]]:
        """
        Проверка наличия критических паттернов SQL-инъекций,
        которые явно указывают на атаку независимо от других факторов.
        """
        if not isinstance(text, str) or len(text) < self.min_query_length:
            return False, []
            
        detected_patterns = []
        for i, pattern in enumerate(self.compiled_critical_patterns):
            if pattern.search(text):
                detected_patterns.append(self.critical_patterns[i])
    ...
    def _is_in_whitelist(self, text: str) -> bool:
        ...
        # Сначала проверяем наличие критических паттернов
        is_critical, _ = self._check_critical_patterns(text)
        if is_critical:
            return False  # Если есть критические паттерны, игнорируем белый список
    ...
    def _detect_regex_patterns(self, text: str) -> Tuple[bool, float, List[str]]:
        ...
        is_critical, critical_patterns = self._check_critical_patterns(text)
        if is_critical:
            return True, 1.0, critical_patterns  # Максимальная оценка для критических паттернов
    ...
    def _calculate_anomaly_score(self, text: str) -> Dict[str, Any]:
        ...
        if is_critical:
            ...
            result['details']['critical_patterns'] = critical_patterns
            return result
        ...
        if component_scores:
            max_component_name, max_component_score = max(component_scores.items(), key=lambda x: x[1])
    ...
    def _generate_explanation(self, text: str) -> str:
        ...
        if anomaly_type == 'SQL Injection':
            ...
            # Сначала проверяем критические паттерны
            critical_patterns = anomaly_result.get('details', {}).get('critical_patterns', [])
            if critical_patterns:
                explanation_parts.append(f"Найдены явные признаки SQL-инъекции ({len(critical_patterns)}):")
                for i, pattern in enumerate(critical_patterns[:3]):  # Показываем только первые 3 паттерна
                    pattern_desc = pattern.replace(r'(?i)', '').replace(r'\b', '').replace(r'\s+', ' ')
                    explanation_parts.append(f"  - Критический паттерн {i+1}: {pattern_desc}")
                
                if len(critical_patterns) > 3:
                    explanation_parts.append(f"  - ... и еще {len(critical_patterns) - 3} критических паттернов")
        ...
        if anomaly_type == 'SQL Injection':
            ...
            explanation_parts.append("- Используйте принцип наименьших привилегий для пользователей базы данных")
            ...
            explanation_parts.append("- Рассмотрите возможность использования хранимых процедур")
        elif anomaly_type == 'Contextual Anomaly':
            explanation_parts.append("- Проверьте запрос на соответствие бизнес-логике приложения")
            explanation_parts.append("- Проведите аудит запросов из этого источника")
